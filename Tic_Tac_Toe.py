...
class TournamentManager(QMainWindow):
    ...
    def round_complete(self, result):
        ...
        if self.current_round < 5:
            self.start_next_round()  # If less than 5 rounds, start next
        else:
            self.match_complete()  # Otherwise, finish match

    def match_complete(self):
        winner = max(self.round_scores, key=self.round_scores.get)
        self.points_table[winner] += 5  # Update points table
        QMessageBox.information(self, "Match Complete", f"{winner} wins the match!")
        self.start_next_match()  # Start the next match

    def end_tournament(self):
        winner = max(self.points_table, key=self.points_table.get)
        QMessageBox.information(self, "Tournament Complete", f"{winner} wins the tournament!")
        self.reset_tournament_data()
        self.update_ui()

    def reset_tournament(self):
        # Reset the entire tournament setup
        self.reset_tournament_data()
        self.update_ui()
        self.game_widget.reset_game()

    def update_ui(self):
        # Update all relevant labels and data in the UI
        self.status_label.setText(f"Tournament Status: {self.tournament_stage.capitalize()}")
        self.current_match_label.setText(f"Current Match: {self.current_match[0]} vs {self.current_match[1]}" if self.current_match else "Current Match: None")
        self.current_round_label.setText(f"Current Round: {self.current_round}/5" if self.tournament_stage == "round-robin" else "Current Round: 0/5")
